...
def addList(*items):
    itemList = list(items)
    print(f'총 갯수 : {len(itemList)}')
    print(f'데이타형 : {type(itemList)}')
    for item in range(len(itemList)):
        print(f'{item} 번째 => {itemList[item]}')
...
def min_max_number(min_max, *nums):
    value = nums[0]
    if min_max == 'min':
        for num in nums:
            if value > num:
