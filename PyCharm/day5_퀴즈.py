...
valueList = ['양준일', '노사연', '구창모', '조용원', '김정화']
def makedict(numKey,value):
    return {'user' + str(numKey) : value}

dictList = list(map(makedict, list(range(1,len(valueList)+1)),valueList))
print(dictList)
...
valueList = ['양준일', '노사연', '구창모', '조용원', '김정화']
dictList = list(map(lambda numKey, value : {'user'+str(numKey) : value}, list(range(1,len(valueList)+1)),valueList))
print(dictList)
...
mKind = ['발라드', '댄스', '클래식', 'OST']
mTitle = ['거리에서', 'DNA', '소녀의 기도', '썸머']
TupleList = list(zip(mKind,mTitle))
print(TupleList)
...
numList = [5, 6, 7, 8, 9, 10]
result = f.reduce(lambda x,y : x*y, numList, 1)
print('5 X 6 X 7 X 8 X 9 X 10  = ', result)
...
result = list(filter(lambda x : not x.isalnum(), myString))
print(result)