...
-- OR 사용
SELECT * FROM buytbl
	WHERE prodName = '청바지' OR prodName = '운동화';
-- IN 사용
SELECT * FROM buytbl
	WHERE prodName IN ('청바지','운동화');
...
-- (서브쿼리 이용)
SELECT * FROM buytbl
	WHERE price > (SELECT price FROM buytbl WHERE userID = 'JYP'); 
    
-- 12) buyTbl 테이블에서 userID 컬럼값이 'JYP'인 amount 컬럼값과 같은 레코드를 출력하여라.
-- (서브쿼리 이용)
SELECT * FROM buytbl
	WHERE amount = (SELECT amount FROM buytbl WHERE userID = 'JYP');
...
SELECT DISTINCT price FROM buytbl ORDER BY price;

-- 18) 17번의 레코드 결과에서 2번째로 큰 가격의 값은?
-- (WHERE, ORDER BY, LIMIT) 이용
SELECT DISTINCT price FROM buytbl 
	ORDER BY price DESC	
	LIMIT 1,1;
...
SELECT * FROM buytbl
	WHERE price = (SELECT DISTINCT price FROM buytbl 
					ORDER BY price DESC	
                    LIMIT 1,1);
